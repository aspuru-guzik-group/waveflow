...
class ModelTrainer:
    ...
    def start_training(self, restart=False):
        """
        ...
        psi, log_pdf, sample, opt_state, opt_update, get_params = create_train_state(self.box_length,
                self.learning_rate, n_particle=self.n_particle, rng=split_rng, xu_coord_type=self.xu_coord_type,
                spline_degree=self.spline_degree, num_knots=self.num_knots, n_flow_layers=self.n_flow_layer)
...
def create_train_state(box_length, learning_rate, n_particle, rng=0, 
                       xu_coord_type='mean', spline_degree=6, num_knots=23,
                       n_flow_layers=3):


    init_fun = get_waveflow_model(n_particle, base_spline_degree=spline_degree, i_spline_degree=spline_degree, 
                                  n_prior_internal_knots=num_knots, n_i_internal_knots=num_knots, 
                                  i_spline_reg=0.05, i_spline_reverse_fun_tol=0.000001,
                                  n_flow_layers=n_flow_layers, box_size=box_length,
...
# def train_step_efficient(psi, h_fn, params, batch, running_average):
#     '''
#     Training step
#     '''
#     loss_val, gradients = value_and_grad(loss_fn_efficient, argnums=0)(params, psi, h_fn, batch, running_average)
