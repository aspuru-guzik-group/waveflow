...
def train_model(rng, params, log_pdf, sample, X, opt_state, num_epochs, 
                batch_size, n_model_sample, save_figs=False, model_type=None,
                check_step=5000):
...
if __name__ == '__main__':
    ...
    if run_all:
        for dataset in dataset_list:
            ...
            for model_type in model_type_list:
                for spline_reg in spline_reg_list:
                    ...
                    train_model(rng, params, log_pdf, sample, X, opt_state, num_epochs, 
                                batch_size, n_model_sample, save_figs=True, 
                                model_type='{}_{}'.format(model_type, spline_reg),
                                check_step=50)
        ...
        train_model(rng, params, log_pdf, sample, X, opt_state, num_epochs, batch_size, 
                    n_model_sample, save_figs=True, 
                    model_type='{}_{}'.format(model_type, spline_reg),
                    check_step=50)
        print('Creating report... ')
        create_figures.create_report('./results/pdf/')
